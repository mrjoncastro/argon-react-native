...
const Header = ({ back, title, white, transparent, bgColor, iconColor, titleColor, optionLeft, optionRight, search, options, tabs, tabIndex, ...props }) => {
  const navigation = useNavigation();
  ...
  const renderRight = () => {
    ...
    switch (title) {
      ...
      case 'Profile':
        return [
          <BellButton key={`chat-${title.toLowerCase()}`} navigation={navigation} isWhite={white} />,
          <BasketButton key={`basket-${title.toLowerCase()}`} navigation={navigation} isWhite={white} />
        ];
      case 'Product':
        return [
